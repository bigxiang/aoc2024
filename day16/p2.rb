...
history = []
queue = [[start_point, 0, direction, history]]
queue_i = 0
best_seats = Set.new
...
while queue_i < queue.size
  curr_point, curr_cost, direction, history = queue[queue_i]
  queue_i += 1
  ...
  next if curr_point != start_point &&
          (existing_cost != '.' &&
           curr_cost > existing_cost &&
           (direction.map { |d| d * -1 } == existing_direction || direction == existing_direction))
  ...
  if curr_point == end_point
    ...
    if curr_cost < min_cost
      puts "new min_cost: #{curr_cost}"
      min_cost = curr_cost

      best_seats = Set.new(history)
      best_seats.add(end_point)

    elsif curr_cost == min_cost
      best_seats.merge(history)
    else
      raise "IMPOSSIBLE, cost too high: #{curr_cost}, min_cost: #{min_cost}"
    end
  ...
  [[-1, 0], [1, 0], [0, -1], [0, 1]].each do |di, dj|
    ...
    step = [[i, j], cost, [di, dj], history + [curr_point]]
...
best_seats.each do |seat_pos|
  puts "best seat: #{seat_pos}"
  map_seats[seat_pos[0]][seat_pos[1]] = 'O'
end

puts map_seats.map { |r| r.join }.join("\n")
